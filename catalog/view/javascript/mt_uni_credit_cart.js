(() => {
  ...
  function init() {
    ...
    function cartChangedMessage() {
      return (
        (state.i18n && state.i18n.cart_changed) ||
    ...
    function isCartListUrl(url) {
      ...
      return (
        normalized.indexOf("route=checkout/cart.list") !== -1 ||
    ...
    function bindCartRefreshListeners() {
      ...
      if ($ && typeof $.fn !== "undefined") {
        $(document).on(
          "ajaxSuccess.mtUniCreditCartRefresh",
        ...
        $("#shopping-cart").on(
          "ajaxComplete.mtUniCreditCartRefresh",
          function () {
    ...
    function hasAuthoritativeCalculation() {
      return !!(
        ...
        cartFingerprint &&
        cartFingerprint ===
          (state.calculator?.cart_fingerprint || cartFingerprint)
      );
    }
    ...
    async function refreshCalculator(reason) {
      ...
      try {
        ...
        if (json.success && json.calculator) {
          ...
          invalidateOpenPopupForCartChange();
          hideCalculatorShell(
            json.error_code === "cart_empty"
              ? cartEmptyMessage()
              : json.message || cartEmptyMessage(),
          );
          if (calculator) {
            calculator.setAttribute("aria-busy", "false");
          }
    ...
    async function submitForm(event) {
      ...
      const activeForm =
        form || modal.querySelector("#mt-uni-credit-cart-form");
      const scheme = selectedScheme();
      const button = submitBtn();
      const submitError = modal.querySelector("[data-mtuc-submit-error]");
