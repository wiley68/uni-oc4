...
(function ($, window, document) {
  ...
  function resolveSecondary(primary, secondaryFromApi, eurMode) {
    if (
      secondaryFromApi !== undefined &&
      secondaryFromApi !== null &&
      String(secondaryFromApi).trim() !== ""
    ) {
  ...
  function setCartButtonBusy(on) {
    var btn = document.getElementById("btn_uni");
  ...
  /** Монотонно при всяко ново преизчисляване — по-бавен отговор не трие по-новия. */
  var uniCreditCartCalcGeneration = 0;
  /** Паралелни заявки: индикаторът до изчерпване на всички текущи AJAX. */
  var uniCreditCartBusyRef = 0;

  function uniCreditAcquireCartButtonBusy() {
    uniCreditCartBusyRef++;
    if (uniCreditCartBusyRef === 1) {
      setCartButtonBusy(true);
    }
  }

  function uniCreditReleaseCartButtonBusy() {
    if (uniCreditCartBusyRef <= 0) {
      return;
    }
    uniCreditCartBusyRef--;
    if (uniCreditCartBusyRef === 0) {
      setCartButtonBusy(false);
    }
  }
  ...
  function updateButton(months, monthly, monthlySecond, eurMode) {
    ...
    if (t) {
      t.textContent =
        Number(monthly || 0).toFixed(2) + " " + (signEl ? signEl.value : "");
    }
    if (t2) {
      if (eurMode === 0 || eurMode === 3 || Number(monthlySecond || 0) === 0) {
        ...
        t2.textContent =
          " (" +
          Number(monthlySecond || 0).toFixed(2) +
          " " +
          (signSecondEl ? signSecondEl.value : "") +
          ")";
      }
    }
  }
  ...
  $(function () {
    ...
    function runCalc() {
      ...
      var calcGen = ++uniCreditCartCalcGeneration;
      uniCreditAcquireCartButtonBusy();
      var months =
        $("#uni_pogasitelni_vnoski_input").val() || cfg.months || "12";
      return calculate(cfg, months)
        .always(function () {
          uniCreditReleaseCartButtonBusy();
        })
        .done(function (json) {
          if (calcGen !== uniCreditCartCalcGeneration) {
            return;
          }
          ...
          var obshtoSecond = resolveSecondary(
            json.uni_obshto,
            json.uni_obshto_second,
            eurMode,
          );
          var mesecnaSecond = resolveSecondary(
            json.uni_mesecna,
            json.uni_mesecna_second,
            eurMode,
          );
    ...
    $(document).on(
      "click.uniCartMtCredit",
      "#uni_buy_on_installment",
      function (e) {
        e.preventDefault();
        var cfg = getCfg();
        if (!cfg) {
    ...
    $(document).ajaxComplete(function (event, xhr, settings) {
      ...
      if (
        url.indexOf("checkout/cart.remove") !== -1 ||
        url.indexOf("checkout/cart.list") !== -1
      ) {
        runCalc();
      }
    });
