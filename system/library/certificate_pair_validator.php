...
/**
 * Validates SmartUCF client certificate + private key PEM pairs (OpenSSL, local only).
 */
final class CertificatePairValidator
{
    private MtlsPrivateKeyPassphraseProvider $passphrases;
     ...
     */
    public function parseCertificateDates(string $certificatePem): array
    {
        $certificate = $this->parseCertificate($this->normalizePem($certificatePem));

        return $this->parseCertificateMetadata($certificate);
    }

    /** @return \OpenSSLCertificate|resource */
    public function parseCertificate(string $certificatePem)
    {
        if (
    ...
    public function parsePrivateKey(string $privateKeyPem)
    {
        if (
     ...
     */
    public function assertKeyMatchesCertificate($certificate, $privateKey): void
    {
