...
final class SchemaFilterMatcher
{
    ...
    public function matches(array $filter, ProductContext $product): bool
    {
        ...
        if (
            $this->months->hasValue($filter['uni_price_from'] ?? null)
            && $product->price < (float) $filter['uni_price_from']
        ) {
            return false;
        }
        if (
            $this->months->hasValue($filter['uni_price_to'] ?? null)
    ...
    {
        if (
            $this->months->hasValue($filter['uni_date_from'] ?? null)
            && $this->today < substr(trim((string) $filter['uni_date_from']), 0, 10)
        ) {
            return false;
        }
        if (
            $this->months->hasValue($filter['uni_date_to'] ?? null)
